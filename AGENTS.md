...
Frontend resources are under `assets/`: the HTML template is in `assets/templates/`, stylesheets in `assets/css/`, and bundled fonts and KaTeX files in their respective subdirectories. Preview tooling lives in `bin/`, Fish completions in `completions/`, and the nginx server block in `nginx/`. Automated Lua regression tests are in `tests/`. Generated previews under `assets/preview/` and manual fixtures under `test/` are ignored.
...
- `bin/pandocmd-preview input.md` rebuilds on changes and triggers browser reload; it requires Pandoc, Fish, `entr`, Python, and configured nginx.
- `make install` links the `ppl` command and its Fish completion into user-local directories.

## Coding Style & Naming Conventions

Follow the surrounding file: Lua uses two-space indentation and `snake_case`; Python and Fish use four spaces, with Python classes in `PascalCase`. Prefer small, single-purpose filter modules. Use `local` for Lua helpers and preserve the custom `util.walk_blocks` traversal semantics. No formatter or linter is configured, so avoid unrelated reformatting.

## Testing Guidelines

Add focused regression cases to `tests/source-line-preprocess.lua`, using descriptive assertion names. Run `make test` for every Lua change. For changes outside that suite, build a representative Markdown fixture and inspect the resulting HTML or live preview. There is no declared coverage threshold; prioritize edge cases involving parsing, math, source lines, and cross-references.
